...
The **MCIR CLI** is a minimal command-line tool designed to publish artifacts into one or more **MCI registries**. It occupies the same conceptual space as tools like `npm publish` or container registry CLIs, but for **MCI specifications** and **MCI modules**.
...
MCIR uses TOML *artifact manifest* files to declare publishable artifacts. These files are always local to the artifact being published and are distinct from the MCIR CLI configuration.
